...
class BladeCompiler extends Compiler implements CompilerInterface
{
    ...
    public function if($name, callable $callback)
    {
        ...
        $this->directive($name, function ($expression) use ($name) {
            return $expression
                    ? "<?php if (\Sirius\Support\Facades\Blade::check('{$name}', {$expression})): ?>"
                    : "<?php if (\Sirius\Support\Facades\Blade::check('{$name}')): ?>";
        });

        $this->directive('else'.$name, function ($expression) use ($name) {
            return $expression
                ? "<?php elseif (\Sirius\Support\Facades\Blade::check('{$name}', {$expression})): ?>"
                : "<?php elseif (\Sirius\Support\Facades\Blade::check('{$name}')): ?>";
        });
